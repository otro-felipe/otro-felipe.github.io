...
- Para resaltar keywords, usa exclusivamente los marcadores `<reaction>`, `<action>`, `<deathknell>`, `<shield n>`, `<assault n>`, `<tank>`, `<deflect>`, `<temporary>` y `<hidden>` (insensibles a mayúsculas); son los únicos reconocidos por el decorador. `<hidden>` utiliza el estilo *green*. Mantén esta política de toma de notas actualizada si cambia.
